...
-- Get node with heigth from nodes without
...
insert x (Node y h lt rt)
    | x < y         = balance y (insert x lt) rt
    | x > y         = balance y lt (insert x rt)
...
updateOrInsert f x (Node y h lt rt)
    | x < y     = balance x (updateOrInsert f x lt) rt
    | x > y     = balance y lt (updateOrInsert f x rt)
    | otherwise = Node (f x) h lt rt

-- We can do insert with updateOrInsert (const x) x
{-

--[ rotR  ]--

rotR (Node x h (Node lk lh llt lrt) rt) = node lk llt (node x lrt rt)
                     --    --- ---             -- ---      -- --- --
                      2     4   5              2   4        1  5   3
          ---  -------------------  --
           1              2          3
...
rotL (Node x h lt (Node rk rh rlt rrt))  =node rk (node x lt rlt) rrt
                        --    --- ---          --      -- -- --   ---                     
                         1     5   3            1      2  4   5    3
          ---  -- 
           2   4  
...
delete x (Node y h lt rt)
    | x < y     = balance y (delete x lt) rt
    | x > y     = balance y lt (delete x rt)
    | otherwise = combine lt rt

combine :: AVL a -> AVL a -> AVL a
combine Empty rt = rt
combine lt Empty = lt
combine lt rt    = balance x lt rt2
    where (x, rt2) = split rt

-- removes and returns minimum element from non-empty tree
split :: AVL a -> (a, AVL a)
split (Node x h Empty rt) = (x, rt)
split (Node x h lt    rt) = (y, balance x lt2 rt)
    where (y, lt2) = split lt


deleteMinim :: AVL a -> AVL a
deleteMinim Empty = error "Can't delete on empty tree"
deleteMinim (Node x h Empty rt) = rt
deleteMinim (node x h lt rt)    = balance x (deleteMinim lt) rt

deleteMaxim :: AVL a -> AVL a
deleteMaxim Empty = error "Can't delete on empty tree"
